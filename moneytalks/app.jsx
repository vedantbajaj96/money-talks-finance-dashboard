...
// ─── Sidebar ──────────────────────────────────────────────────────
function Sidebar({ active, onChange, layout, syncState }) {
...
function TopBar({ tab, monthKey, setMonthKey, search, setSearch, syncState, syncing, syncProgress, manualSync, isAdmin }) {
  ...
  useEffect(() => {
    ...
    const handler = (e) => { if (e.key === 'Escape') setShowMonthPicker(false); };
  ...
  return (
    ...
    <header className="topbar">
      <div className="topbar-left">
        ...
        {showMonth && (() => {
          // Group months by year for the popover grid
          const yearGroups = {};
          MONTHS.forEach(m => {
...
function App() {
  ...
  // Expose a global toast function so any tab can trigger it
  useEffect(() => {
  ...
  // Document title — update whenever the active tab changes
  useEffect(() => {
